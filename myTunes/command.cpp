...
int Command::getNumTokens() { return tokens.size(); }

vector<string> Command::getTokens() { return tokens; }
...
bool Command::isCompareCommand() {
	if(getCommandName().compare(CMD_COMPARE) == 0) return true;
	return false;
}
bool Command::isReorderCommand() {
	if(getCommandName().compare(CMD_REORDER) == 0) return true;
	return false;
}
bool Command::isFollowCommand() {
	if(getCommandName().compare(CMD_FOLLOW) == 0) return true;
	return false;
}
