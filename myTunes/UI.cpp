...
void UI::run(){
	...
	while (true) {
		...
		if(input.empty()) {/*do nothing*/}
			...
			else if(cmd.isCompareCommand()) owner->executeCommand(cmd);
			else if(cmd.isReorderCommand()) owner->executeCommand(cmd);
			else if(cmd.isFollowCommand()) owner->executeCommand(cmd);
...
void UI::executeCommand(Command cmd){
	...
	else if(cmd.isCommand(CMD_COMPARE)) owner->executeCommand(cmd);
}
